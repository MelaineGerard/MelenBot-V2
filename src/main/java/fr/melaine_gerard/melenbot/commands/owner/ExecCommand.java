...
public class ExecCommand implements ICommand {
    ...
    public void handle(GuildMessageReceivedEvent event, List<String> args) {
        if(args.isEmpty()) {
            event.getChannel().sendMessage(EmbedUtils.createErrorEmbed(event.getJDA(), "Merci d'indiquer une commande").build()).queue();
            return;
        }
        ...
        try {
            ...
            if(resultat.length() <= 2048)
                event.getChannel().sendMessage(EmbedUtils.createSuccessEmbed(event.getJDA(), result.toString()).build()).queue();
            else{
                ...
                event.getChannel().sendMessage(EmbedUtils.createSuccessEmbed(event.getJDA(), "Résultat trop long " + hasteUrl).build()).queue();
